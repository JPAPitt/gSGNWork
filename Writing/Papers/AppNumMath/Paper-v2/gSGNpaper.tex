...
\fancyhf{}
\rfoot{\emph{\footnotesize \textcopyright  gSGNE method by  J. Pitt, C. Zoppou and S. Roberts.}
...
%Since the numerical scheme's foundation is a finite volume method for an equation in conservation law shown to work well for nonlinear non-dispersive equations \cite{Kurganov-Petrova-2007-707} this scheme should be valid for all values of $\beta_1$ and $\beta_2$ without specialised modification. 
...
\begin{figure}
	...
	\caption{Rectangular negative wave experiments ($h_0 - h_1 = 0.03m$) time series at the five Wave Gauges (WG) for simulation with the improved SGNE with $\beta_1 = 4/5$ and $\beta_2 = 2/15$ (\solidrule), the classical SGNE with $\beta_1 = 2/3$ and $\beta_2 = 0$ ({\color{blue}\solidrule}), the SWWE with $\beta_1 = 0$ and $\beta_2 = 0$ ({\color{green!80!black}\dashedrule}) compared to the experimental results ({\color{red}\solidrule}). The left time series show the entire duration with  highlighted region ({\color{gray}\dashedrule}) shown on the right. }
