...
%DIF DEL gSGNpaper _v1.tex   Thu Dec 16 04:19:32 2021
%DIF ADD gSGNpaper.tex       Wed Jun 15 14:56:30 2022
...
\fancyhf{}
%DIF 38c38
%DIF < \rfoot{\emph{\footnotesize \textcopyright  gSGN Method by  J. Pitt, C. Zoppou and S. Roberts.}
%DIF -------
\rfoot{\emph{\footnotesize \textcopyright  gSGNE method by  J. Pitt, C. Zoppou and S. Roberts.} %DIF > 
%DIF -------
...
%DIF < 	\includegraphics{Figure-1abcde.pdf}
%DIF < \end{graphicalabstract}
%DIF < 
%DIF < 
%DIF < \begin{keyword}
%DIF < 	generalised Serre-Green-Naghdi \sep Finite Volume Method \sep Finite Difference Method \sep Serre equations \sep regularised Shallow Water Wave Equations \sep dispersive water wave equations.
%DIF < \end{keyword}
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Linearised gSGNE}
...
%DIFDELCMD < %%%
\DIFdelend \subsubsection{Dispersion Relation}
...
\label{eq:FDGeqn}
\end{equation}
where $\vecn{A}$ is a tri-diagonal matrix with the sub-diagonal, diagonal and super-diagonal elements \DIFdelbegin \DIFdel{are }\DIFdelend given by
\begin{subequations}
\begin{align}
...
%DIF < 
%DIF > Better match discussion of results to Figures!!!!!
\begin{figure}
	\centering
	\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}{0.49\textwidth}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figure-11.pdf}
	\caption{\DIFaddFL{Rectangular negative wave experiments ($h_0 - h_1 = 0.01m$) time series at the five Wave Gauges (WG) for simulation with the improved SGNE with $\beta_1 = 2/3 + 2/15$ and $\beta_2 = 2/15$ (}\solidrule\DIFaddFL{), the classical SGNE with $\beta_1 = 2/3$ and $\beta_2 = 0$ (}{\color{blue}\solidrule}\DIFaddFL{), the SWWE with $\beta_1 = 0$ and $\beta_2 = 0$ (}{\color{green!80!black}\dashedrule}\DIFaddFL{) compared to the experimental results (}{\color{red}\solidrule}\DIFaddFL{). The left time series show the entire duration with  highlighted region (}{\color{gray}\dashedrule}\DIFaddFL{) shown on the right. }}
	\label{Fig:Neg_Wave_1cm}
\end{figure}

\begin{figure}
	\DIFaddendFL \centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Figure-11a.pdf}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figure-12.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{$h_0 - h_1 = 0.01m$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Rectangular negative wave experiments ($h_0 - h_1 = 0.03m$) time series at the five Wave Gauges (WG) for simulation with the improved SGNE with $\beta_1 = 4/5$ and $\beta_2 = 2/15$ (}\solidrule\DIFaddFL{), the classical SGNE with $\beta_1 = 2/3$ and $\beta_2 = 0$ (}{\color{blue}\solidrule}\DIFaddFL{), the SWWE with $\beta_1 = 0$ and $\beta_2 = 0$ (}{\color{green!80!black}\dashedrule}\DIFaddFL{) compared to the experimental results (}{\color{red}\solidrule}\DIFaddFL{). The left time series show the entire duration with  highlighted region (}{\color{gray}\dashedrule}\DIFaddFL{) shown on the right. }\DIFaddendFL }
	\DIFdelbeginFL %DIFDELCMD < \label{Fig:Neg_Wave_1}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}{0.49\textwidth}
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=\textwidth]{Figure-11b.pdf}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{$h_0 - h_1 = 0.03m$}}
		%DIFAUXCMD
%DIFDELCMD < \label{Fig:Neg_Wave_2}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Rectangular negative wave experiments time series at the five Wave Gauges (WG) for simulation with the improved SGN with $\beta_1 = 2/3 + 2/15$ and $\beta_2 = 2/15$ (}%DIFDELCMD < \solidrule%%%
\DIFdelFL{), the classical SGN with $\beta_1 = 2/3$ and $\beta_2 = 0$ (}%DIFDELCMD < {\color{blue}\solidrule}%%%
\DIFdelFL{), the SWWE with $\beta_1 = 0$ and $\beta_2 = 0$ (}%DIFDELCMD < {\color{green!80!black}\dashedrule}%%%
\DIFdelFL{) compared to the experimental results (}%DIFDELCMD < {\color{red}\solidrule}%%%
\DIFdelFL{).}}
	%DIFAUXCMD
%DIFDELCMD < \label{Fig:Neg_Wave}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{Fig:Neg_Wave_3cm}
...
\section{Conclusion}
%DIF < include experimental results
