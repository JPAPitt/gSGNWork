...
c ===== 
     ...
     . pubc_init(xbc_len),hbc_fin(xbc_len),
     . ubc_fin(xbc_len),hbc_fin_a(xbc_len),
     . ubc_fin_a(xbc_len),
     . Energs_init(3), Energs_fin(3),Energ_Err(3),
     . Norms(2),sumerr(2),suma(2)
      INTEGER i
     ...
     . ga,a0,a1,hbc_init,ubc_init)
     
      ! generate intial previous values of h and u, so h^{-1} and u^{-1}
      CALL SGNSolitaryTravellingWave(xbc,xbc_len,tstart - dt,
     . ga,a0,a1,phbc_init,pubc_init)
     
      
      !Numerically Solve gSGN with beta values until currenttime > tend     
      CALL NumericalSolveTSPrint(tstart,tend,
     . ga,beta1,beta2,dx,dt,n_GhstCells,xbc_len,xbc,
     . hbc_init,ubc_init,
     . phbc_init,pubc_init,
...
c ===== 
     ...
     . dx,dt,Cr,maxwavespeed,beta1,beta2,alpha
