...
def PlotBuildUpToQuad(x,q,dx,nG,np):
    ...
    for i in range(nG,n-nG):
        ...
        P2jm3tojm1P = P2jm3tojm1a22*(xplot - x[i-1])**2 + P2jm3tojm1a21*(xplot - x[i-1])   +P2jm3tojm1a20

        
        if (abs(PlusCAErr -MinusCAErr)<eps):
            
            #Pick lowest order one
            if(abs(P2jm2toja22)< eps ):
                Ra2= P2jm2toja22
                Ra1= P2jm2toja21
                Ra0= P2jm2toja20
            elif(abs(P2jm1tojp1a22) < eps):
                Ra2= P2jm1tojp1a22
                Ra1= P2jm1tojp1a21
                Ra0= P2jm1tojp1a20   
            elif(abs(P2jtojp2a22)< eps):
                Ra2= P2jtojp2a22
                Ra1= P2jtojp2a21
                Ra0= P2jtojp2a20
            else:     
        ...
        RaP2 =  Ra2*(xplot - x[i])**2  + Ra1*(xplot - x[i]) + Ra0

        # print(i,xmh,xph,'CA P1 j+1',P1jp1tojp2a10 - qaj)
        # print(i,xmh,xph,'CA P1 j-1',P1jm2tojm1a10 - qaj)
        ...
        if i == nG:
            ...
            plot(xplot, P2jm2tojP, '-b',label='Recon P2 -')     
            plot(xplot, P2jm1tojp1P, '-g',label='Recon P2 0')   
            plot(xplot, P2jtojp2P, '-r',label='Recon P2 +')   
            
            plot(xplot, RaP2, '-c',label='Recon P2 +')  

            plot(xplot, P2jp1tojp3P, '--y',label='Recon P2 + In')            
            plot(xplot, P2jm3tojm1P, '--m',label='Recon P2 - In')   
            ...
            plot(xplot, P2jtojp2P, '-r')   
            
            plot(xplot, RaP2, '-c')  

            plot(xplot, P2jp1tojp3P, '--y')            
            plot(xplot, P2jm3tojm1P, '--m')   
