...
def EXPPeakS_IC(x):
    ...
    for i in range(n):
        xmjh = x[i] - dx/2
        xpjh = x[i] + dx/2
        qamjh = 4*xmjh - ((xmjh )**5)/5.0
        qapjh = 4*xpjh - ((xpjh )**5)/5.0
        q[i] = (qapjh - qamjh)/ dx
    return q  
...
def PlotBuild3(x,q,dx,nG,np):
    ...
    for i in range(nG,n-nG):
        ...
        #Right
        if I1 == 1:
            if(P2jtojp2SI == min(P2jm2tojSI,P2jm1tojp1SI, P2jtojp2SI )  ):
                Ra2 = P2jtojp2a22
                Ra1 = Ra1 - dx*Ra2
                Ra0 = Ra0 - dx**2/12*Ra2
            else:
        ...
        #Left
            ...
            if(P2jm2tojSI  == min(P2jm2tojSI,P2jm1tojp1SI, P2jtojp2SI ) ):
                Ra2 = P2jm2toja22
                Ra1 = Ra1 + dx*Ra2
                Ra0 = Ra0 - dx**2/12*Ra2
            else:
        ...
        LimR = Ra3*(xplot - x[i])**3 + Ra2*(xplot - x[i])**2 + Ra1*(xplot - x[i])   +Ra0
        ...
        if i == nG:          
            plot(xplot, LimR, '-r',label='Recon P3 j-3,j+3')
        else:
            plot(xplot, LimR, '-r')
