...
def PlotBuildUpToQuad(x,q,dx,nG,np):
    ...
    for i in range(nG,n-nG):
        ...
        PlusCAErrp3 = abs((RIxjph -  RIxjmh)/dx-qaj )
        print(i,xmh,xph,'P+ in CA p3', PlusCAErrp3 )
        ...
        MinusCAErrp3 = abs((RIxjph -  RIxjmh)/dx-qaj )
        print(i,xmh,xph,'P- in CA p3', MinusCAErrp3 )
        ...
        if (abs(PlusCAErrp0) < eps or abs(MinusCAErrp0) < eps ):
            ...
            if (abs(PlusCAErrp1) < eps):
                ...
                if (abs(PlusCAErrp2) < eps):
                    Ra2= P2jtojp2a22
                    Ra1= P2jtojp2a21
                    Ra0= P2jtojp2a20
                elif (abs(MinusCAErrp2) < eps ):
                    Ra2= P2jm2toja22
                    Ra1= P2jm2toja21
                    Ra0= P2jm2toja20
                else:
                    if (min(PlusCAErrp3,MinusCAErrp3) < min(PlusCAErrp2,MinusCAErrp2,PlusCAErrp1,MinusCAErrp1,PlusCAErrp0,MinusCAErrp0)):
                        if (abs(PlusCAErrp3 -MinusCAErrp3)<eps):   
                            if ( PlusCAErrp3 < min(MinusCAErrp2,MinusCAErrp1,MinusCAErrp0,PlusCAErrp0,PlusCAErrp1,PlusCAErrp2)):
                                a66,a65,a64,Ra3,Ra2,Ra1,Ra0,SI = P6jm3tojp3(qajm3,qajm2,qajm1,qaj,qajp1,qajp2,qajp3,dx)
                            else:
                                Ra0 = qaj
                        elif(PlusCAErrp3 > MinusCAErrp3):
                            Ra3 = pjm3toja33
                            Ra2 = pjm3toja32
                            Ra1 = pjm3toja31
                            Ra0 = pjm3toja30
                        ...
                        Ra0 = qaj
        RaP = Ra3*(xplot - x[i])**3  + Ra2*(xplot - x[i])**2 +  Ra1*(xplot - x[i]) + Ra0
